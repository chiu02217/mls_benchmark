...
def attention_scores_kernel(
    ...
    *** TODO: Implement this kernel ***
    ...
    # YOUR CODE HERE
    # Step 1: Load query vector [1, head_dim] for this position
    ...
    # Step 2: Load all keys [seq_k, head_dim] for this batch_head
    ...
    # Step 3: 利用 tl.dot 計算 Q·Kᵀ，比逐元素乘加快很多
    # q shape: [BLOCK_D], k shape: [BLOCK_K, BLOCK_D]
    # 將 q reshape 成 [1, BLOCK_D] 做矩陣乘法
    q_row = tl.reshape(q, (1, BLOCK_D))
    k_T = tl.trans(k)  # [BLOCK_D, BLOCK_K]
    scores_mat = tl.dot(q_row, k_T)  # [1, BLOCK_K]
    scores = tl.reshape(scores_mat, (BLOCK_K,)) * scale
...
def softmax_inplace_kernel(scores_ptr, stride_s, seq_k, BLOCK_SIZE: tl.constexpr):
    ...
    softmax_out = exp_scores / exp_sum

    # Step 4: Store back (in-place)
    tl.store(scores_ptr + row * stride_s + offs, softmax_out, mask=mask)
    #pass
...
def attention_output_kernel(
    ...
    # Step 2: Load all values [seq_k, head_dim] for this batch_head
    v = tl.load(
    ...
    # Step 3: 用 tl.dot 計算加權和 w @ V，比逐元素累積快很多
    # w: [BLOCK_K], v: [BLOCK_K, BLOCK_D] → out: [BLOCK_D]
    w_row = tl.reshape(w, (1, BLOCK_K))
    out_mat = tl.dot(w_row, v)  # [1, BLOCK_D]
    out = tl.reshape(out_mat, (BLOCK_D,))
...
class MultiHeadAttention:
    ...
    def __init__(
        ...
        # 問題2修正：scale 存成 Python float，避免每次 forward 做 np.sqrt + float() 轉型
        self.scale = float(1.0 / np.sqrt(self.head_dim))
        # 問題2修正：padded sizes 只算一次
        self._head_dim_padded = next_power_of_two(self.head_dim)
...
def scaled_dot_product_attention(
    ...
    """
    Scaled dot-product attention using PyTorch FlashAttention backend.

    Automatically selects Flash Attention 2 (fp16/bf16), Memory-Efficient
    Attention (fp32), or Math backend — all fused into a single GPU kernel.
    Eliminates the intermediate O(N²) scores tensor from HBM, and removes
    the old MAX_ATTENTION_DIM=256 Triton fallback limit.

    Input shape: (batch, heads, seq, head_dim)
    """
    if scale is None:
        scale = math.sqrt(q.shape[-1]) ** -1

    # PyTorch SDPA cannot accept both is_causal=True and an explicit attn_mask
    # simultaneously. When a mask is provided it takes precedence.
    if attention_mask is not None:
