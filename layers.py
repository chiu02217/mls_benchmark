...
def rmsnorm_kernel(
    ...
    *** TODO: Implement this kernel ***
    ...
    # YOUR CODE HERE
    pid = tl.program_id(0)
     # Step 1: 建立 offset 並 load 輸入 row 和 weight
    ...
    # Step 2: 計算 variance = mean(x^2)，使用 float32 累積精度
    x_sq_sum = tl.sum(x * x, axis=0)
    variance = x_sq_sum / hidden_size

    # Step 3: 正規化 x / sqrt(variance + eps)
    rstd = 1.0 / tl.sqrt(variance + eps)
    x_norm = x * rstd

    # Step 4: 套用 weight 並 store
    y = x_norm * w
...
def gelu_kernel(x_ptr, y_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
    ...
    *** TODO: Implement this kernel ***
    ...
    # Step 2: 使用 tanh 近似公式計算
    # GELU(x) = 0.5 * x * (1 + tanh(sqrt(2/pi) * (x + 0.044715 * x^3)))
    inner = 0.79788456 * (x + 0.044715 * x * x * x)
...
def linear_kernel_tf32(
    ...
    *** TODO: Implement this kernel ***
    ...
    # Pre-compute M/N boundary masks once — they don't depend on k
    mask_m = offs_m < M  # (BLOCK_M,)
    mask_n = offs_n < N  # (BLOCK_N,)

    accumulator = tl.zeros((BLOCK_M, BLOCK_N), dtype=tl.float32)
    for k in range(0, K, BLOCK_K):
        mask_k = k + offs_k < K  # only active on the last K-tile
        a = tl.load(
...
class Linear:
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self._weight_t = None  # cached transposed weight for Triton kernel
        # self._K_padded = None
        # self._N_padded = None
        self._K_padded = pad_to_multiple(in_features, self.TILE_K)
        self._N_padded = pad_to_multiple(out_features, self.TILE_N)
    ...
    #         #weight_t = self.weight.t().to(device=self.weight.device, dtype=torch.float32).contiguous()
    ...
    def _ensure_weight_prepared(self):
        """Cache transposed weight without unnecessary padding."""
    ...
    #     return output.reshape(*batch_dims, self.out_features)
    def _forward_triton(self, x: torch.Tensor) -> torch.Tensor:
        """Triton matmul backend (Optimized)."""
        original_shape = x.shape
        batch_dims = original_shape[:-1]
...
class MLP:
    ...
    def _forward_fused(self, x: torch.Tensor) -> torch.Tensor:
        """Fused SwiGLU forward pass (Optimized)."""
        
        # ====== 這裡就是消失的權重搬移檢查 ======
        # 必須確保權重跟輸入 x 在同一個 GPU 設備上！
        if self.gate_proj.weight.device != x.device:
            self.gate_proj.weight = self.gate_proj.weight.to(x.device)
            self._gate_weight_t = None
        if self.up_proj.weight.device != x.device:
        ...
        # 優化：直接配置剛好大小的 intermediate，讓 mask 去處理邊界
...
class EncoderMLP:
    ...
    def _forward_fused(self, x: torch.Tensor) -> torch.Tensor:
        """Fused Linear+GELU forward pass (Optimized)."""
        
        # ====== 權重與輸入的 Device 同步檢查 ======
        # 確保第一層的權重跟輸入 x 都在同一個 GPU 設備上
        if self.fc1.weight.device != x.device:
            self.fc1.weight = self.fc1.weight.to(x.device)
            self._fc1_weight_t = None
        self._prepare_fused_weights()
        ...
        # 優化：直接宣告 Exact Size 的記憶體，把邊界處理 (Padding) 的工作完全交給 Triton Kernel 內的 Mask
        intermediate = torch.empty((M, N), dtype=torch.float32, device=x.device)
        ...
        if self.bias_enabled and self.fc1.bias_param is not None:
            if self.fc1.bias_param.device != x.device:
                self.fc1.bias_param = self.fc1.bias_param.to(x.device)
            intermediate += self.fc1.bias_param

        # Reshape 回原本的 Batch 維度並送入第二層 (第二層的 fc2 是 Linear 物件，內部會自己處理 Device 同步)
        intermediate = intermediate.reshape(*orig_shape[:-1], self.intermediate_size)
        return self.fc2(intermediate)
